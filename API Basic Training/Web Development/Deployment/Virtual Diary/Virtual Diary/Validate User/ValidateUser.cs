...
namespace Virtual_Diary
{
    ...
    internal class ValidateUser
    {
        ...
        public static bool IsLogin(string username, string password)
        {
            ...
            bool isUser = BLUser.lstUsers.Any(user => user.UserName.Equals(username) && user.Password == password);
            return isUser;
        }
        ...
        public static User GetUserDetails(string username, string password)
        {
            ...
            User objUSer = BLUser.lstUsers.FirstOrDefault(user => user.UserName.Equals(username) && user.Password == password);
            return objUSer;
        }
    }
}
